...
// end:source ../src/umd-head.js
	...
	jMask.prototype = {
		...
		text: function(mix, ctx, controller){
			...
			for(var i = 0, x, imax = this.length; i < imax; i++){
				x = this[i];
				string += jmask_getText(x, mix, ctx, controller);
			}
			return string;
		}
	};
	...
	arr_each(['appendTo'], function(method) {
		...
		jMask.prototype[method] = function(mix, model, ctx, controller) {
	
			if (controller == null) 
				controller = this;
			
			if (mix.nodeType != null && typeof mix.appendChild === 'function') {
				mix.appendChild(this.render(model, ctx, null, controller));
